package de.cobolj.parser.statement.multiply;

import java.util.ArrayList;
import java.util.List;
import java.util.stream.Collectors;

import de.cobolj.nodes.ExpressionNode;
import de.cobolj.nodes.PictureNode;
import de.cobolj.parser.Cobol85BaseVisitor;
import de.cobolj.parser.Cobol85Parser.MultiplyByStatementContext;
import de.cobolj.parser.statement.CalculationResult;
import de.cobolj.parser.statement.LiteralOrIdentifierVisitor;
import de.cobolj.parser.statement.add.ResultIdentifierVisitor;
import de.cobolj.statement.multiply.MultiplyByStatementNode;

/**
 * multiplyByStatement: MULTIPLY literalOrIdentifier BY resultIdentifier+
 * 
 * @author flaechsig
 *
 */
public class MultiplyByStatementVisitor extends Cobol85BaseVisitor<MultiplyByStatementNode> {

	@Override
	public MultiplyByStatementNode visitMultiplyByStatement(MultiplyByStatementContext ctx) {
		List<ExpressionNode> left = new ArrayList<>();
		ExpressionNode right;
		List<CalculationResult> results;
		List<PictureNode> slots = new ArrayList<>();
		List<Boolean> roundeds = new ArrayList<>();

		left.add(ctx.literalOrIdentifier().accept(new LiteralOrIdentifierVisitor()));
		results = ctx.resultIdentifier().stream().map(result -> result.accept(new ResultIdentifierVisitor()))
				.collect(Collectors.toList());

		for (CalculationResult singleResult : results) {
			slots.add(singleResult.slot);
			roundeds.add(singleResult.rounded);
		}

		return new MultiplyByStatementNode(left, slots, roundeds);
	}
}
